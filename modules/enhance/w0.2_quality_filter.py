...
def filter_prompts(input_list, max_workers):
    ...
    stats = {
        ...
        "quality_filtered": 0,  # 质量不合格过滤掉的数量
        "passed": 0,  # 最终通过的数量
    }

    # Step 4: 质量检查
    judge_list = parallel_wrapper(
        preprocess_list(input_list), check_quality, max_workers=max_workers
    )
    quality_prompts = [p for p, flag in zip(input_list, judge_list) if flag]
    stats["quality_filtered"] = len(input_list) - len(quality_prompts)
    stats["passed"] = len(quality_prompts)
    logger.info(f"Step 4 --- quality_filtered: {stats}")

    # Write final results
    writejsonl(quality_prompts, args.output_file)
    writejson(stats, args.output_stats_file)

    return quality_prompts, stats
